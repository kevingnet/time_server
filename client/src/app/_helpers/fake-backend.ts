...
export class FakeBackendInterceptor implements HttpInterceptor {
    intercept(request: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>> {
        ...
        function handleRoute() {
            switch (true) {
                case url.endsWith('/login') && method === 'POST':
                    return login();
                case url.endsWith('/current_time') && method === 'GET':
                    return currentTime();
        ...
        function login() {
            ...
            const user = users.find(x => (x.email === email || x.username === email) && x.password === password);
        ...
        function currentTime() {
            return ok([{ id: '1', time: new Date().toISOString(), token: 'demo' }]);
        }
